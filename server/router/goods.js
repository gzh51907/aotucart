...
router.post("/change", async (req, res) => {
  ...
  let result = update(
    "goodslist_all",
    { name: new RegExp(psname, "g") },
    {
      price: price,
      kucun: kucun,
      current_price: currentprice,
      goods_brief: info,
      default_photo: majorimg
    }
  );
...
router.post("/addsubscribe", async (req, res) => {
  let { city, phone, name, purpose } = req.body;

  let result = await create("q_subscribe", [{ city, phone, name, purpose }]);
...
router.get("/keyword", async (req, res) => {
  ...
  let result = await find("goodslist_all", {
...
router.get("/all/img", async (req, res) => {
  ...
  result.forEach((item, i) => {
    ...
    // 将图片写入本地
    request(item.default_photo.thumb).pipe(
      fs.createWriteStream("../webapp/src/assets/img/" + filename)
    );
